	...
	for (; *format != 0; ++format) {
		if (*format == '%') {
			...
			if( *format == 'f' ) {
				...
				pc += printi (out, wholePart, 10, 1, width, pad, 'a', buflimit);
				printchar (out, '.', buflimit);
				double diff = number - wholePart;
				int fractionalPart = diff > 0 ? diff * 1e3 : (-diff) * 1e3;
				pc += printi (out, fractionalPart, 10, 0, width, pad, 'a', buflimit);
				continue;
			}
			if( *format == 'x' ) {
